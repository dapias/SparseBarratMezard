...
for T in T_grid
    @JLD2.load "/Users/diegotapias/cavity/data/gillespie/T=$(T)c=$(c).jld2" trees
    ensemble = length(trees)
    es = zeros(length(t_grid), ensemble);
    
    for i in 1:ensemble
        slices = Array{Node}(undef, length(binning));
    ...
    save("data/gillespie/energy/T=$(T)c=$(c).jld",  "binning", binning, "meane", meane, "ensemble", ensemble)
end
